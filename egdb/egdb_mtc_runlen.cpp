...
static int initdblookup(DBHANDLE *hdat, int pieces, int cache_mb, char *filepath, void (*msg_fn)(char *))
{
	...
	for (i = 0; i < hdat->numdbfiles; ++i) {
		...
		sprintf(dbname, "%s%s.cpr_mtc", hdat->db_filepath, hdat->dbfiles[i].name);
	...
	if (hdat->cacheblocks) {
		hdat->ccbs = (CCB *)malloc(hdat->cacheblocks * sizeof(CCB));
		if (!hdat->ccbs) {
...
static int parseindexfile(DBHANDLE *hdat, DBFILE *f, int *allocated_bytes)
{
	...
	sprintf(name, "%s%s.cpr_mtc", hdat->db_filepath, f->name);
	...
	sprintf(name, "%s%s.idx_mtc", hdat->db_filepath, f->name);
	fp = fopen(name, "r");
	if (fp == 0) {
