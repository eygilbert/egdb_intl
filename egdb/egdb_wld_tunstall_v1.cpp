...
static int dblookup(EGDB_DRIVER *handle, EGDB_POSITION *p, int color, int cl)
{
	...
	if (dbpointer->file->file_cache) {
		...
		if (node) {
			...
			ccbp = update_lru<DBHANDLE, DBFILE, CCB>(hdat, dbpointer->file, node->cacheblock_index);
		}
		else {
...
#define MAX_AUTOLOAD_RATIO .35
	...
	if (i > 0) {
		...
		if (hdat->cache_ht_size > 0) {
			hdat->cache_ht = (CACHE_HASHTABLE_NODE **)malloc(hdat->cache_ht_size * sizeof(hdat->cache_ht[0]));

			hdat->cache_ht_nodes_base = (CACHE_HASHTABLE_NODE *)malloc(hdat->cacheblocks * sizeof(hdat->cache_ht_nodes_base[0]));
			...
			while (1) {
				...
				if (stat == '#') {
					...
					if (strstr(line, "vmap")) {
						...
						stat = sscanf(line, " vmap %d: %d,%d,%d,%d,%d,%d",
